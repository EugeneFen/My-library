...
namespace My_library
{
    public partial class Form1 : Form
    {
            ...
            if ((text1 != "") && (text2 != ""))
                if(root.CheckRot()) //не пустое дерево
                    if(root.Poisk(text1, text3, text2)) //такая книга есть
                        {
                            root.Delete(text1, text2, true); //удаляем
                ...
                MessageBox.Show("Нет ни одного элемента", "Сообщение");
            else
                MessageBox.Show("Поля оказались пусты", "Сообщение");
        }
        ...
        private void treeAVL_AfterSelect(object sender, TreeViewEventArgs e) //отладка авл дерева
        {
